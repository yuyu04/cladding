...
export async function runInit(opts: InitOptions = {}): Promise<InitResult> {
  ...
  if (companionLang && !getHostMcpServer()) {
    ...
    const viewPath = `docs/project-context.${companionLang}.md`;
    if (translator) {
      try {
        const langName = langDisplayName(companionLang);
        const prompt = [
          `Translate the following Markdown document to ${langName} faithfully.`,
          'Preserve all Markdown structure, headings, code fences, and identifiers',
          '(e.g. F-1a2b3c) verbatim. Output ONLY the translated Markdown.',
          '',
          '---',
          projectContextMd,
        ].join('\n');
        ...
        if (translated.length > 0) {
          const header =
            '<!-- GENERATED VIEW — NOT THE SSoT. Canonical: docs/project-context.md ' +
            `(${langDisplayName(canonicalLang())}). Regenerated by clad init/refine; ` +
            'do not edit by hand. -->\n\n';
          ...
          process.stderr.write(
            `[clad init] wrote localized view ${viewPath} via ${i18nModel()}\n`,
          );
          appendEvent(
            cwd,
            newEvent('spec_view_generated', {
              ...
              path: viewPath,
              charsCanonical: projectContextMd.length,
